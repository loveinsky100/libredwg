...
static void
common_entity (Dwg_Object_Entity *ent, string_builder *sb)
{
  double lweight;
  char *color;
  lweight = entity_lweight (ent);
  color = entity_color (ent);
  sb_appendf (sb, "      style=\"fill:none;stroke:%s;stroke-width:%.1fpx\" />\n",
          color, lweight);
  if (*color == '#')
    free (color);
}
...
// TODO: MTEXT
static void
output_TEXT (Dwg_Object *obj, string_builder *sb)
{
  ...
  sb_appendf (sb, "\t<text id=\"dwg-object-%d\" x=\"%f\" y=\"%f\" "
...
static void
output_LINE (Dwg_Object *obj, string_builder *sb)
{
  ...
  sb_appendf (sb, "\t<!-- line-%d -->\n", obj->index);
  sb_appendf (sb, "\t<path id=\"dwg-object-%d\" d=\"M %f,%f L %f,%f\"\n\t", obj->index,
          transform_X (start.x), transform_Y (start.y), transform_X (end.x),
          transform_Y (end.y));
  common_entity (obj->tio.entity, sb);
}

static void
output_XLINE (Dwg_Object *obj, string_builder *sb)
{
...
static void
output_RAY (Dwg_Object *obj, string_builder *sb)
{
...
static void
output_CIRCLE (Dwg_Object *obj, string_builder *sb)
{
  ...
  sb_appendf (sb, "\t<!-- circle-%d -->\n", obj->index);
  sb_appendf (sb, "\t<circle id=\"dwg-object-%d\" cx=\"%f\" cy=\"%f\" r=\"%f\"\n\t",
...
static void
output_POINT (Dwg_Object *obj, string_builder *sb)
{
  ...
  sb_appendf (sb, "\t<!-- point-%d -->\n", obj->index);
  sb_appendf (sb, "\t<circle id=\"dwg-object-%d\" cx=\"%f\" cy=\"%f\" r=\"0.1\"\n\t",
          obj->index, transform_X (pt1.x), transform_Y (pt1.y));
  common_entity (obj->tio.entity, sb);
}

static void
output_ARC (Dwg_Object *obj, string_builder *sb)
{
...
// FIXME
static void
output_ELLIPSE (Dwg_Object *obj, string_builder *sb)
{
  ...
  */
  ...
  sb_appendf (sb, "\t<!-- ellipse-%d -->\n", obj->index);
  sb_appendf (sb, "\t<!-- sm_axis=(%f,%f,%f) axis_ratio=%f start_angle=%f "
          "end_angle=%f-->\n",
          ell->sm_axis.x, ell->sm_axis.y, ell->sm_axis.z, ell->axis_ratio,
          ell->start_angle, ell->end_angle);
  sb_appendf (sb, "\t<ellipse id=\"dwg-object-%d\" cx=\"%f\" cy=\"%f\" rx=\"%f\" "
...
// untested
static void
output_SOLID (Dwg_Object *obj, string_builder *sb)
{
  ...
  sb_appendf (sb, "\t<!-- solid-%d -->\n", obj->index);
  sb_appendf (sb, "\t<polygon id=\"dwg-object-%d\" "
...
// untested
static void
output_3DFACE (Dwg_Object *obj, string_builder *sb)
{
  ...
  if (isnan_3BD (ent->corner1) || isnan_3BD (ent->corner2)
      ...
      || entity_invisible (obj))
    return;
  sb_appendf (sb, "\t<!-- 3dface-%d -->\n", obj->index);
  if (ent->invis_flags)
    {
      // move to 1
      sb_appendf (sb, "\t<path id=\"dwg-object-%d\" d=\"M %f,%f", obj->index,
              ent->corner1.x, ent->corner1.y);
      sb_appendf (sb, " %s %f,%f", ent->invis_flags & 1 ? "M" : "L", ent->corner2.x,
              ent->corner2.y);
      sb_appendf (sb, " %s %f,%f", ent->invis_flags & 2 ? "M" : "L", ent->corner3.x,
              ent->corner3.y);
      sb_appendf (sb, " %s %f,%f", ent->invis_flags & 4 ? "M" : "L", ent->corner4.x,
              ent->corner4.y);
...
static void
output_POLYLINE_2D (Dwg_Object *obj, string_builder *sb)
{
  int error;
      ...
      for (j = 1; j < numpts; j++)
        {
          ...
          sb_appendf (sb, " L %f,%f", transform_X (pt.x), transform_Y (pt.y));
        }
...
static void
output_LWPOLYLINE (Dwg_Object *obj, string_builder *sb)
{
  int error;
...
// TODO: MINSERT
static void
output_INSERT (Dwg_Object *obj, string_builder *sb)
{
  ...
  if (insert->block_header && insert->block_header->handleref.value)
    {
      ...
      sb_appendf (sb, "\t<!-- insert-%d -->\n", obj->index);
      sb_appendf (sb, "\t<use id=\"dwg-object-%d\" transform=\"translate(%f %f) "
...
static int
output_object (Dwg_Object *obj, string_builder *sb)
{
  int num = 1;
  if (!obj)
    {
  ...
  switch (obj->fixedtype)
    ...
    case DWG_TYPE_INSERT:
      output_INSERT (obj, sb);
      break;
    case DWG_TYPE_LINE:
      output_LINE (obj, sb);
      break;
    case DWG_TYPE_CIRCLE:
      output_CIRCLE (obj, sb);
      break;
    case DWG_TYPE_TEXT:
      output_TEXT (obj, sb);
      break;
    ...
    case DWG_TYPE_ELLIPSE:
      output_ELLIPSE (obj, sb);
      break;
    case DWG_TYPE_SOLID:
      output_SOLID (obj, sb);
      break;
    case DWG_TYPE__3DFACE:
      output_3DFACE (obj, sb);
      break;
    case DWG_TYPE_POLYLINE_2D:
      output_POLYLINE_2D (obj, sb);
      break;
    case DWG_TYPE_LWPOLYLINE:
      output_LWPOLYLINE (obj, sb);
      break;
    case DWG_TYPE_RAY:
      output_RAY (obj, sb);
      break;
...
static int
output_BLOCK_HEADER (Dwg_Object_Ref *ref, string_builder *sb)
{
  Dwg_Object *obj;
  ...
  if (hdr->name)
    {
      ...
      if (!escaped || strcmp (escaped, "*Model_Space") != 0)
        {
          is_g = 1;
          sb_appendf (sb, "\t<g id=\"symbol-" FORMAT_RLLx "\" >\n\t\t<!-- %s -->\n",
...
void dwg2svg (Dwg_Data *dwg, string_builder *sb)
{
  ...
  // broken rng
  sb_appendf (sb, "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>\n"
          "<svg\n"
  ...
  if (!mspace && (ref = dwg_paper_space_ref (dwg)))
    num = output_BLOCK_HEADER (
        ref, sb); // how many paper-space entities we did print
  if (!num && (ref = dwg_model_space_ref (dwg)))
    output_BLOCK_HEADER (ref, sb);
  sb_appendf (sb, "\t<defs>\n");
